...
- `package.json` – scripts for dev, build, lint, test and API client/spec generation.
  Run `bun run apigen` to regenerate `api/api-spec.json` and refresh `frontend/src/generated/api`.
...
- `frontend/src/generated/api/` – typed client regenerated by `bun run apigen`.
